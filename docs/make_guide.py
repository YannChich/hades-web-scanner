...
VULNS = [
    ...
    module("xss_detect", "Cross-Site Scripting (XSS)",
        "Injects a marker, finds exactly where it is reflected (HTML, attribute, JS, comment), then sends the minimal context-specific breakout and checks the special characters survive unencoded.",
        "Reflected XSS runs the attacker's JavaScript in a victim's browser.",
        "Session-cookie theft, actions performed as the victim, page defacement — account takeover when cookie flags are weak.",
        "Context analysis keeps it credible: an encoded reflection is Low, a real breakout is High."),
