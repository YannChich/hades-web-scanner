...
VULNS = [
    ...
    module("xss_detect", "Cross-Site Scripting (XSS) — reflected, stored & DOM-based",
        "Three passes: (1) reflected — find where a marker is reflected (HTML, attribute, JS, comment) and send a context-specific breakout; (2) stored — submit a form, then re-check display pages for the value, not just the submission's own response; (3) browser-verified — an optional headless-browser pass that injects a benign payload via a form field, the URL #fragment, or a reflected query parameter, and confirms it actually executes in a real browser. This catches DOM sinks (innerHTML, location.hash) that never show in the server response AND event-handler-attribute reflected XSS (onload=\"...('param')...\") where the server entity-encodes the quote but the browser decodes it before running the JS.",
